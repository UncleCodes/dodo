...
import com.dodo.privilege.entity.monitor_2.log_1.SlowSqlLog;
...
public class StatFilter extends FilterEventAdapter implements StatFilterMBean {
    ...
    public String mergeSql(String sql, String dbType) {
        ...
        try {
            ...
            LOG.error("merge sql error, dbType " + dbType + ", druid-" + VERSION.getVersionNumber() + ", sql : " + sql,
                    e);
        }
    ...
    private final void internalAfterStatementExecute(StatementProxy statement, boolean firstResult,
        ...
        if (sqlStat != null) {
            ...
            if (millis >= slowSqlMillis) {
                ...
                if (!statement.getLastExecuteSql().contains("slow_sql_log")) {
                    taskExecutor.execute(new Runnable() {
                        @Override
                        public void run() {
                            try {
                                SlowSqlLog slowSqlLog = new SlowSqlLog();
                                slowSqlLog.setCompareMillis(slowSqlMillis);
                                slowSqlLog.setExecuteMillis(millis);
                                slowSqlLog.setExecuteSql(CommonUtil.escapeHtml(lastExecSql));
                                slowSqlLog.setSlowParameters(CommonUtil.escapeHtml(slowParameters));
                                ((HqlHelperService) SpringUtil.getBean("hqlHelperServiceImpl")).save(slowSqlLog);
                            } catch (Exception e) {
                                LOG.error("Can't load bean 'hqlHelperServiceImpl' from Spring Root WebApplicationContext");
                            }
                        }
                    });
                }
    ...
    protected void resultSetOpenAfter(ResultSetProxy resultSet) {
        JdbcDataSourceStat dataSourceStat = resultSet.getStatementProxy().getConnectionProxy().getDirectDataSource()
                .getDataSourceStat();
    ...
    @Override
    public <T> T resultSet_getObject(FilterChain chain, ResultSetProxy result, int columnIndex, Class<T> type)
            throws SQLException {
    ...
    @Override
    public <T> T resultSet_getObject(FilterChain chain, ResultSetProxy result, String columnLabel, Class<T> type)
            throws SQLException {
