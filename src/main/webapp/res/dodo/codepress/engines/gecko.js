...
CodePress = {
	...
	keyHandler : function(evt) {
		...
		//add by dingjh execSQl
		else if(keyCode== 13 && evt.ctrlKey)  {
			if(!document.getElementsByTagName('pre')[0] || editor.innerHTML == ''){
				window.parent.$.dodo.sqlreport.execSQL(CodePress.getCode(),1, CodePress.getEditor().innerHTML);
			}else{
				window.parent.$.dodo.sqlreport.execSQL(CodePress.getCode(),1, editor.innerHTML);
			}
		}
	...
	syntaxHighlight : function(flag) {
		...
		x = window.parent.$.dodo.sqlreport.makeHighLightSQL(x);
