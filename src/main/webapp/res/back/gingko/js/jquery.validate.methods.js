...
jQuery.validator.addMethod("sysUse",function(value, element){return true;}, "页面加载错误");
jQuery.validator.addMethod("isMobile",function(value, element){return this.optional(element) || (/^1\d{10}$/.test(value));}, "mobile needed");
(function(){
	...
	jQuery.validator.addMethod("requiredShowOn",function( value, element, param ){
		var classStr = $(element).parents("td").attr("class");
		var showonvalues = $(element).parents("td").attr("showonvalues");
		if(classStr==undefined||classStr.indexOf("showon_")==-1){
			classStr = $(element).parents("tr").attr("class");
			showonvalues = $(element).parents("tr").attr("showonvalues");
		}
		var fieldId = classStr.split(" ")[0].replace("showon_","");
		if($("#"+fieldId).val()==""||showonvalues.indexOf(","+$("#"+fieldId).val()+",")==-1){
			return true;
		}
